...
import { Camera, Save, User, Mail, Phone, Building, Briefcase, Shield, Calendar, Loader2, CheckCircle, Globe } from 'lucide-react'
...
export default function Profile() {
  ...
  // In dev, load images directly from Flask to avoid Vite proxy binary corruption
  const imgBase = import.meta.env.DEV ? 'http://localhost:3399' : ''
  const avatarUrl = profile?.profile_picture ? `${imgBase}/api/uploads/${profile.profile_picture}` : null
  ...
  const orgInfo = auth.user?.org_name
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <motion.div
        ...
        className={cn('rounded-2xl border p-6 shadow-card overflow-hidden relative', dark ? 'border-navy-700 bg-navy-800' : 'border-gray-200 bg-white')}
      >
        {/* Decorative gradient strip */}
        <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-r from-[#0a1628] via-[#0d2a5e] to-[#1a3a6e] opacity-90" />

        <div className="relative flex flex-col items-center gap-4 sm:flex-row sm:items-end pt-8 sm:pt-10">
          {/* Avatar */}
          <div className="relative group">
            <div className="flex h-28 w-28 items-center justify-center rounded-2xl text-3xl font-bold shadow-lg overflow-hidden ring-4 ring-white dark:ring-navy-800 bg-gradient-to-br from-brand-dark to-brand-mid text-white relative">
              {initials}
              {avatarUrl && !imgError && (
                <img
                  src={avatarUrl}
        ...
        {profile?.email && (
          <div className="mt-5 flex flex-wrap gap-2">
            <span className={cn('flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs', dark ? 'bg-navy-700 text-brand-muted' : 'bg-gray-50 text-gray-500')}>
            ...
            {profile.department && (
              <span className={cn('flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs', dark ? 'bg-navy-700 text-brand-muted' : 'bg-gray-50 text-gray-500')}>
                <Building size={12} /> {profile.department}
              </span>
            )}
            <span className={cn('flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs', dark ? 'bg-green-900/30 text-green-400' : 'bg-green-50 text-green-600')}>
      ...
      <motion.div
        ...
        className={cn('rounded-2xl border p-6 shadow-card', dark ? 'border-navy-700 bg-navy-800' : 'border-gray-200 bg-white')}
      >

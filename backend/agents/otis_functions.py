...
class OtisFunctionRegistry:
    ...
    def _register_all_functions(self):
        ...
        # ── Live Search & External Data ────────────────────────────────────────

        self.register(FunctionDefinition(
            name="search_hotels",
            description="Search hotels and accommodation in a city",
            function=self._search_hotels_wrapper,
            parameters={
                "destination": "City to search hotels in",
                "duration_days": "Number of nights (default: 3)",
                "budget": "Budget tier: budget / moderate / luxury (default: moderate)",
            },
            required_params=["destination"],
            admin_only=False,
            examples=[
                "Find hotels in Mumbai",
                "Search hotels in Bangalore for 5 nights",
                "Budget accommodation in Delhi",
                "Where can I stay in Chennai?",
            ]
        ))

        self.register(FunctionDefinition(
            name="search_transport",
            description="Search flights, trains, or buses between two cities",
            function=self._search_transport_wrapper,
            parameters={
                "destination": "Destination city",
                "origin": "Departure city (optional)",
                "preferred_mode": "flight / train / bus / cab (optional)",
            },
            required_params=["destination"],
            admin_only=False,
            examples=[
        ...
        self.register(FunctionDefinition(
            ...
            function=self._get_weather_wrapper,
            parameters={
                "city": "City name",
            },
        ...
        self.register(FunctionDefinition(
            ...
            function=self._convert_currency_wrapper,
            parameters={
            ...
            examples=[
                ...
                "How much is 500 euros in rupees?",
                "Currency rate for Dubai",
                "500 AED in INR",
            ]
        ))
        ...
        self.register(FunctionDefinition(
            name="plan_trip",
            description="Plan a trip to a destination (guides user to the Planner tab)",
            function=self._plan_trip_wrapper,
            parameters={
                "destination": "Destination city",
                "duration_days": "Number of days (default: 3)",
            },
            required_params=["destination"],
            admin_only=False,
            examples=[
                ...
                "Help me plan travel to Bangalore",
                "I need to visit Chennai next week",
            ]
        ))

        self.register(FunctionDefinition(
            name="send_sos",
            description="Send an SOS emergency alert to managers",
            function=self._send_sos_wrapper,
            parameters={
                "city": "Current city or location",
                "emergency_type": "Type: medical / fire / theft / security / general",
            ...
            examples=[
                "Send SOS",
                "Emergency help in Mumbai",
                "SOS from Delhi — medical emergency",
                "Send alert — I need help",
            ]
        ))
    ...
    # ── Live Search Wrappers (delegate to otis_voice_dispatcher) ──────────────

    async def _search_hotels_wrapper(self, context: Dict, params: Dict) -> Dict:
        try:
            from agents.otis_voice_dispatcher import _handle_hotel_search
            user = {"id": context["user_id"], "role": context.get("user_role", "employee")}
            command = f"find hotels in {params.get('destination', '')} for {params.get('duration_days', 3)} nights {params.get('budget', 'moderate')} budget"
            result = _handle_hotel_search(user, command)
            return {"success": True, "data": result.get("query_data", {}), "voice_response": result["response_text"]}
        except Exception as e:
            return {"success": False, "error": str(e), "voice_response": "I couldn't search hotels right now."}

    async def _search_transport_wrapper(self, context: Dict, params: Dict) -> Dict:
        try:
            from agents.otis_voice_dispatcher import _handle_transport_search
            user = {"id": context["user_id"], "role": context.get("user_role", "employee")}
            origin = params.get("origin", "")
            dest = params.get("destination", "")
            mode = params.get("preferred_mode", "")
            command = f"find {mode or 'transport'} {'from ' + origin if origin else ''} to {dest}"
            result = _handle_transport_search(user, command)
            return {"success": True, "data": result.get("query_data", {}), "voice_response": result["response_text"]}
        except Exception as e:
            return {"success": False, "error": str(e), "voice_response": "I couldn't find transport options right now."}

    async def _get_weather_wrapper(self, context: Dict, params: Dict) -> Dict:
        try:
            from agents.otis_voice_dispatcher import _handle_weather
            user = {"id": context["user_id"]}
            command = f"weather in {params.get('city', '')}"
            result = _handle_weather(user, command)
            return {"success": True, "data": result.get("query_data", {}), "voice_response": result["response_text"]}
        except Exception as e:
            return {"success": False, "error": str(e), "voice_response": "I couldn't get the weather right now."}

    async def _convert_currency_wrapper(self, context: Dict, params: Dict) -> Dict:
        try:
            from agents.otis_voice_dispatcher import _handle_currency
            user = {"id": context["user_id"]}
            amount = params.get("amount", 1)
            from_cur = params.get("from_currency", "USD")
            to_cur = params.get("to_currency", "INR")
            command = f"convert {amount} {from_cur} to {to_cur}"
            result = _handle_currency(user, command)
            return {"success": True, "data": result.get("query_data", {}), "voice_response": result["response_text"]}
        except Exception as e:
            return {"success": False, "error": str(e), "voice_response": "Currency conversion unavailable right now."}

    async def _plan_trip_wrapper(self, context: Dict, params: Dict) -> Dict:
        try:
            from agents.otis_voice_dispatcher import _handle_plan_trip
            user = {"id": context["user_id"]}
            dest = params.get("destination", "")
            dur = params.get("duration_days", 3)
            command = f"plan a {dur} day trip to {dest}"
            result = _handle_plan_trip(user, command)
            return {"success": True, "data": result.get("query_data", {}), "voice_response": result["response_text"]}
        except Exception as e:
            return {"success": False, "error": str(e), "voice_response": "I couldn't set up a trip plan right now."}

    async def _send_sos_wrapper(self, context: Dict, params: Dict) -> Dict:
        try:
            from agents.otis_voice_dispatcher import _handle_sos
            user = {"id": context["user_id"], "name": context.get("user_name", "")}
            city = params.get("city", "")
            etype = params.get("emergency_type", "general")
            msg = params.get("message", "")
            command = f"sos {etype} {msg} in {city}".strip()
            result = _handle_sos(user, command)
            return {"success": True, "data": result.get("query_data", {}), "voice_response": result["response_text"]}
        except Exception as e:
            return {"success": False, "error": str(e), "voice_response": "SOS alert sent. Call 112 if urgent."}

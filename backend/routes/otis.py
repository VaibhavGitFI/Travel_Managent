...
# ── Lazy singletons — one instance per process, not per request ───────────────
_stt_service = None
_tts_service = None
...
def _naturalise_for_tts(text: str) -> str:
    """
    Make OTIS text sound like natural spoken Indian English.

    - Expands abbreviations TTS engines stumble over
    - Converts ₹ / $ / € to spoken words so they aren't skipped
    - Keeps numbers short (avoids digit-by-digit reading)
    - Removes leftover markdown fragments
    - Adds a soft pause after long sentences for more natural delivery
    """
    import re as _re
    ...
    text = _re.sub(r'₹\s*([\d,]+(?:\.\d+)?)', r'\1 rupees', text)
    text = _re.sub(r'\$\s*([\d,]+(?:\.\d+)?)', r'\1 dollars', text)
    text = _re.sub(r'€\s*([\d,]+(?:\.\d+)?)', r'\1 euros', text)
    text = _re.sub(r'£\s*([\d,]+(?:\.\d+)?)', r'\1 pounds', text)

    # ── Common abbreviations ───────────────────────────────────────────────
    abbrevs = {
        r'\bkm\b': 'kilometres', r'\bKM\b': 'kilometres',
        r'\bkph\b': 'kilometres per hour',
        r'\bmin\b': 'minutes', r'\bhr\b': 'hours', r'\bhrs\b': 'hours',
        r'\bINR\b': 'rupees', r'\bUSD\b': 'dollars',
        r'\bEUR\b': 'euros', r'\bGBP\b': 'pounds',
        r'\bAED\b': 'dirhams', r'\bSAR\b': 'riyals',
        r'\bpax\b': 'passengers', r'\bpax\b': 'passengers',
        r'\bpct\b': 'percent', r'\b%\b': ' percent',
        r'\bvs\b\.?': 'versus', r'\betc\b\.?': 'and so on',
...
def _get_service_status() -> dict:
    ...
    try:
        provider = _get_stt().get_active_provider()
        if provider:
    ...
    try:
        provider = _get_tts().get_active_provider()
        if provider:
...
def process_command_rest(user):
    ...
    try:
        ...
        try:
            ...
            # 1. Try action dispatcher (hotel, flights, weather, currency, SOS, plan)
            from agents.otis_voice_dispatcher import dispatch as _otis_dispatch
            dispatched_result = _otis_dispatch(user, command_text)

            # 2. Try structured DB query (trips, expenses, meetings, approvals, analytics)
            structured_result = None
            if not dispatched_result:
                structured_result = _resolve_structured_otis_query(user, command_text)

            context = None
            conversation_history = []
            if not dispatched_result and not structured_result:
        ...
        if dispatched_result:
            response_text = dispatched_result["response_text"]
            query_type = dispatched_result["query_type"]
            query_data = dispatched_result["query_data"]
            data_source = dispatched_result["data_source"]
...
def transcribe_audio(user):
    ...
    try:
        ...
        def _run_stt():
            ...
            try:
                return loop.run_until_complete(stt.transcribe(audio_bytes, mime_type))
            finally:
                loop.close()
        ...
        # Empty transcript = silence or no speech — do NOT fall through to Gemini.
        # Gemini multimodal STT is not designed for real-time voice commands and
        # takes 28-45 s on ambient audio, hallucinating garbage text.
        logger.info("[OTIS Transcribe] Empty transcription — audio silent or too short")
        return jsonify({"success": False, "silent": True, "text": "",
                        "error": "No speech detected"}), 200

    except Exception as e:
        logger.warning(f"[OTIS Transcribe] STT service failed: {e}")

    # STT service raised an exception (network issue, timeout, etc.)
    return jsonify({"success": False, "error": "Speech recognition unavailable — try again"}), 422
...
def speak_text(user):
    ...
    data = request.get_json(silent=True) or {}
    text = _naturalise_for_tts(data.get("text", "").strip()[:2000])
    ...
    try:
        ...
        if result and result.audio_data:
            ...
            # Beep is not real speech — signal the frontend to use browser TTS instead.
            # Returning audio here would play a tone and fool the frontend into thinking
            # TTS succeeded, blocking the SpeechSynthesis fallback.
            if provider == "beep":
                logger.info("[OTIS Speak] beep-only TTS — instructing frontend to use browser TTS")
                return jsonify({"success": False, "error": "TTS unavailable",
                                "use_browser_tts": True}), 503
